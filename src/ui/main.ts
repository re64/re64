    ...
    for (const tok of sorted) {
      ...
      switch (tok.kind) {
        ...
        case "label":
          ...
          // Absent for an invented or built-in name, which is what tells the
          // editor it is naming rather than renaming. An address cannot
          // identify a claim, so a rename has to go by id.
          if (tok.claimId) attrs["data-claim"] = tok.claimId;
...
interface EditTarget {
  ...
  isNew: boolean;
  /**
   * The claim being renamed, when one exists and is editable.
   *
   * Its presence is the whole distinction between renaming and naming: a rename
   * corrects a claim by id, and a name with no claim behind it — an invented
   * `dat_XXXX`, a built-in platform name, or a bare address — can only be added.
   */
  claimId?: string;
}
...
async function commitLabelEdit(target: EditTarget, value: string): Promise<void> {
  ...
  if (!name) {
    // Only a stored claim can be removed, and only by id. Clearing an invented
    // `dat_8100` removes nothing because nothing stored it — which is right,
    // and used to delete whichever claim an address happened to resolve to.
    const removing = target.claimId;
    if (target.isNew || !removing) return;
    await edit(
      (s) => s.removeClaim(removing),
      () => `Removed ${target.name} at ${hex4(target.address)}`
    );
    return;
  }

  // Correcting goes by id; naming adds. Renaming an invented `dat_8100` has no
  // claim to correct, so it adds one — which is right, because nothing stored
  // the invented name and there is nothing there to rewrite.
  const claimId = target.claimId;
  await edit(
    (s) => (claimId ? s.setClaim(claimId, { name }) : s.addLabel(target.address, name, undefined)),
    () => `${claimId ? "Renamed" : "Added"} ${name} at ${hex4(target.address)}`
  );
}
...
function toggleFunctionOnCurrentLine(): void {
  ...
  // Both directions are `src/core/ops/edits.ts`'s job, and were reimplemented
  // here — including a second copy of `isAutoGeneratedName`. The core versions
  // correct by id and already encode the two rules this used to restate: a
  // promoted `loc_` moves to `sub_`, and clearing a name the disassembler would
  // generate anyway removes it rather than leaving an untyped duplicate.
  const address = row.address;
  if (!isFunction) {
    void edit(
      (s) => s.markFunction(address),
      (delta) =>
        `${hex4(address)} is a function` +
        (delta > 0 ? ` — ${delta} more instructions` : "")
    );
    return;
  }

  void edit(
    (s) => s.unmarkFunction(address),
    (delta) =>
      `${name} is no longer a function` +
      (delta < 0 ? ` — ${-delta} fewer instructions` : "")
  );
}
...
function nameCurrentLine(): void {
  ...
  if (labelLine >= 0) {
    ...
    beginLabelEdit({
      ...
      to: token.end,
      isNew: false,
      ...(token.claimId ? { claimId: token.claimId } : {}),
    });
    return;
  }
    ...
    if (rename !== null) {
      beginLabelEdit({
        ...
        ...(el.getAttribute("data-claim")
          ? { claimId: el.getAttribute("data-claim")! }
          : {}),
      });
      event.preventDefault();
      return true;
...
/**
 * Which target is being read.
 *
 * The last surface to learn that targets exist, and the gap was in the way of
 * the ordinary case rather than an exotic one: on a project built by running a
 * loader, the disassembly worth reading is in the *runtime* target and the
 * default is whatever the project opened with. Without this a person watching
 * agents work could not see what they were working on.
 *
 * Choosing writes nothing. `session.selectTarget` holds it locally — which one
 * I am reading is a cursor, not a fact about the project — so two windows can
 * sit on two targets of one document, which is the case that makes a shared
 * selection wrong.
 */
function renderTargets(): void {
  const picker = $("#target") as HTMLSelectElement;
  ...
  for (const target of targets) {
    ...
    option.selected = target.name === current;
    picker.appendChild(option);
  }
  // One target is what a single-layer project has, and a control offering no
  // choice is noise — the same rule the project picker follows.
  picker.style.display = targets.length > 1 ? "" : "none";
}

$("#target").addEventListener("change", (event) => {
  const wanted = (event.target as HTMLSelectElement).value;
  try {
    session?.selectTarget(wanted);
  } catch {
    // A target removed by somebody else between the paint and the click. Put
    // the list back rather than leaving a selection that means nothing.
    renderTargets();
    return;
  }
  // A different view is a different program: the address under the cursor may
  // not exist in it, so this repaints from the top rather than restoring an
  // anchor into a listing that has been replaced.
  render();
  renderTargets();
});
...
function bindPixelControls(): void {
  ...
  $("#px-declare").addEventListener("click", () => {
    ...
    void edit(
      (s) =>
        s.addClaim(pixelAddress, {
          says: { is: "bitmap", view: `${pixelFormat}:${pixelWidth}` },
          extent: length,
        }),
...
async function loadDisassembly(restoreAddress?: number): Promise<void> {
  ...
  session.onChange(() => {
    // A collaborator can add or remove a target, so the list is re-derived on
    // every change rather than only at connect.
    renderTargets();
    scheduleRepaint();
  });
  session.onPresence(() => {
