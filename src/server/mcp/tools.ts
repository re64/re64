...
export function registerTools(rawServer: unknown, context: () => McpContext): void {
  ...
  const tool = (
    ...
    server.registerTool(
      ...
      {
        description,
        // **A target only where a target means something.** It used to be
        // injected onto every tool, "declared here rather than seventy times",
        // and a tool advertising an argument it cannot use is a lie in the one
        // place agents read — one that could break a call whose meaning it could
        // not change. `views.ts` holds the rule and the borderline cases.
      ...
      async (args: never) => {
        ...
        // Which view answered, on every answer that had one. Seeing it is the
  ...
  tool(
    ...
    (args: { project?: string; claim?: string }) =>
      context().workspace(args.project).evidenceFor(args.claim)
  );
  ...
  tool(
    ...
    "Start a project. The first step when you have been handed a binary and no " +
      ...
      "$E000 returns, and an emulator that can run a KERNAL call instead of " +
      "falling into unmapped memory. They are *declared*, not linked from " +
      "whatever this host happens to have on disk, so the project says the same " +
      "thing everywhere and one without the bytes reports romsMissing and opens " +
      "anyway. A ROM is read through rather than read: it costs no listing and " +
      "no coverage. Omit it for a project that is not a C64 program at all.",
    {
      name: z.string().min(1),
      platform: z
        .enum(["c64"])
        .optional()
        .describe("Declare this machine's ROMs. Omitted: nothing but the name."),
    },
    ({ name, platform }: { name: string; platform?: "c64" }) =>
      context().workspace().createProject(name, platform)
  );
  ...
  tool(
    ...
    ({ project: id, target }: { project?: string; target?: string }) => {
      const { workspace, caller } = context();
      return workspace(id, target).undo(caller);
    }
  );
}
