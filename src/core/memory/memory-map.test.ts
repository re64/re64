...
describe("MemoryMap", () => {
  ...
  describe("regions belong to their layer", () => {
    it("resolves the region from the layer that supplies the byte", () => {
      ...
      lower.regions.addRegion(spanClaim({ id: "rgn_1000", start: 0x1000, end: 0x1010, kind: "text", name: "lowerText" }));
      upper.regions.addRegion(spanClaim({ id: "rgn_1000", start: 0x1000, end: 0x1010, kind: "jumptable", name: "upperTable" }));
    ...
    it("falls back to the layer default outside any declared region", () => {
      ...
      layer.regions.addRegion(spanClaim({ id: "rgn_1000", start: 0x1000, end: 0x1010, kind: "text" }));
      map.addLayer(layer);
    ...
    it("resolves nested regions innermost-first", () => {
      const map = new MemoryMap();
      const layer = constLayer("layer", 0x1000, 0x100, 0x00);
      layer.regions.addRegion(spanClaim({ id: "rgn_1000", start: 0x1000, end: 0x1080, kind: "data", name: "outer" }));
      layer.regions.addRegion(spanClaim({ id: "rgn_1010", start: 0x1010, end: 0x1020, kind: "text", name: "inner" }));
      map.addLayer(layer);
    ...
    it("attachRegion gives the region to the topmost covering layer", () => {
      ...
      const owner = map.attachRegion(spanClaim({ id: "rgn_1000", start: 0x1000, end: 0x1010, kind: "text" }));
    ...
    it("attachRegion reports unmapped regions instead of dropping them silently", () => {
      ...
      expect(map.attachRegion(spanClaim({ id: "rgn_9000", start: 0x9000, end: 0x9010, kind: "data" }))).toBeUndefined();
    });
  });
});
