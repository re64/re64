...
/**
 * The generated module's text, so it can be produced without being written.
 *
 * Split from the script that writes it purely so a test can regenerate and
 * compare. A generated file that is committed goes stale in silence otherwise —
 * and this one is not inert data any more: the clobber sets feed the value
 * analysis, so an unnoticed drift between the ROM and the table would show up as
 * a wrong flag proof somewhere else entirely.
 */
export function generateKernalEffects(romPath: string): {
  source: string;
  entries: number;
  vectored: number;
  routines: number;
...
`;
  ...
  return { source, entries: rows.length, vectored: vectoredCount, routines: routines.size, instructions: result.instructions.size, clobbers: clobbers.length };
}
